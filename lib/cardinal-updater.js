...
function installCardinal(cb) {
    ...
    run('npm install')().then(function () {
        process.chdir(currentDir);
        cb(RecursionDeletion('./cardinal', './themes/default', '../cardinal/release/cardinal'));
    });
}
...
function RecursionDeletion(folderPath) {
    console.log('Deleting the files that will be replaced by the build:');
    let folder;
    if (typeof folderPath == Array) {
        folder = folderPath.pop();
    } else {
        folder = folderPath
    }
