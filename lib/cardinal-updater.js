...
async function deleteFiles(cb) {
    ...
    try {
        RecursionDeletion('./cardinal');
        RecursionDeletion('./themes/default')
        RecursionDeletion('../' + CARDINAL_PATH + '/release/cardinal')
        fs.unlink('./cardinal.js');
    } catch (err) {
        console.log(err);
    }
    ...
    fs.readdir(folderPath, function (err, files) {
        if (err) {
            ...
            while (files.length != 0) {
                ...
                if (fs.lstat(folderPath + path).isDirectory()) {
                    RecursionDeletion(folderPath + path)
                } else {
                    fs.unlink(folderPath + path);
                }
            }
        }
    })
