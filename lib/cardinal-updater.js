...
function copyBuild() {
    ...
    try {
        ...
        // })
        const a =RecursionCopy(['../' + CARDINAL_PATH + '/release/cardinal.js', '../' + CARDINAL_PATH + '/release/cardinal', '../' + CARDINAL_PATH + '/release/themes/'], ['./', './cardinal/', './themes/'],true)
        console.log(a)
    } catch (err) {
...
function RecursionCopy(arraySourcePaths, destinationPaths, multipleDestinations) {
    ...
    if (!multipleDestinations) {
        if (Array.isArray(arraySourcePaths)) {
            outputBuffer = arraySourcePaths.map((path) => {
                return gulp.src(path).pipe(dest(destinationPaths));
            });
        } else {
            outputBuffer = gulp.src(arraySourcePaths).pipe(dest(destinationPaths));
        }
    ...
    return arraySourcePaths.map((path, index) => {
        return gulp.src(path).pipe(dest(destinationPaths[index]));
    });
}
...
//                     const statSync = fs.lstatSync(folder + '/' + path)
//                     if (statSync.isDirectory()) {
//                         RecursionCopy(folder + '/' + path, destination + '/' + path)
//                         console.log(folder + '/' + path, destination)
//                     } else {
//                         fs.copyFile(folder + '/' + path, destination + path, (err) => {
//                             console.log(err)
//                         });
//                         console.log(folder + '/' + path, destination + path)
//                     }
//                 }
//             }
//         })
//     } else {
//         const file = folder.split('/')
//         fs.copyFile(folder, file.pop(), (err) => {
//             console.log(err)
//         });
//     }
//     if (typeof folderPath !== 'string') {
...
//             RecursionCopy(folderPath, destinationPath)
//         }
//     }
